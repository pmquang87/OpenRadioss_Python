...
Fortran origin:
  ...
  - Shell kernel:   none upstream (solids only; shell_update raises NotImplementedError;
                    starter ANCMSG 305, hm_read_mat14.F:151-158; mulawc.F90:1125-1307)
...
def tangent(group: Any = None, sig: Optional[np.ndarray] = None, **kwargs: Any) -> Optional[np.ndarray]:
    ...
    Dispatches to consistent_solid_tangent for 3D solid elements.
    Shell elements are not supported upstream (mulawc.F90:1125-1307, hm_read_mat14.F:151-158).
    """
    if group is None:
        return None
