"""OpenRadioss /MAT/LAW113 — Yield Curve Fitting & Hardening Optimizer.

Advanced non-linear spring and beam connector material model (/MAT/SPR_BEAM) with
6 coupled degrees of freedom, tabulated loading/unloading hardening curves, velocity-
dependent damping, strain-rate sensitivity, and damage rupture criteria.

Upstream Fortran references:
  - `starter/source/materials/mat/mat113/hm_read_mat113.F`
  - `starter/source/materials/mat/mat113/law113_upd.F`
  - `hm_cfg_files/config/CFG/radioss2020/MAT/mat113_spr_beam.cfg`
  - `hm_cfg_files/config/CFG/radioss2023/MAT/mat113_spr_beam.cfg`
"""
...
@dataclass
class BeamSpringDOF:
    """Properties for one degree of freedom in /MAT/LAW113."""
    stiff: float = 1.0
    damp: float = 0.0
    acoeft: float = 1.0
    bcoeft: float = 0.0
...
@dataclass
class Law113Params:
    """Parameters for OpenRadioss /MAT/LAW113 (/MAT/SPR_BEAM)."""
    id: int = 1
    title: str = ""
    rho0: float = 0.0
    refer_rho: float = 0.0
    ...
    ifail2: int = 0
    iequil: int = 0
    dofs: List[BeamSpringDOF] = field(default_factory=lambda: [BeamSpringDOF() for _ in range(6)])
    ...
    def __post_init__(self) -> None:
        ...
        if len(self.dofs) < 6:
            self.dofs.extend([BeamSpringDOF() for _ in range(6 - len(self.dofs))])
    ...
    @classmethod
    def from_material(cls, mat: Any) -> Law113Params:
        """Construct Law113Params from generic Material or dictionary."""
        if isinstance(mat, Law113Params):
            return mat
...
def law113_upd(p: Law113Params, curve_dict: Optional[Dict[int, Tuple[np.ndarray, np.ndarray]]] = None) -> None:
    """Fit tangent stiffness and force offset matching `law113_upd.F`."""
        ...
        if d.fun_a > 0 and d.fun_a in curve_dict:
            ...
            if len(x_pts) >= 2:
                ...
                for j in range(len(x_pts)):
                    if abs(x_pts[j]) < 1.0e-12:
                        d.f_x0 = float(y_pts[j])
                        break


def build_law113(mat: Any = None, **kwargs: Any) -> Law113Params:
    """Construct Law113Params from material or keyword arguments."""
    if mat is not None:
        p = Law113Params.from_material(mat)
        for k, v in kwargs.items():
            if hasattr(p, k):
                setattr(p, k, v)
        p.__post_init__()
        return p
    valid_keys = {f.name for f in Law113Params.__dataclass_fields__.values() if f.init}
...
def resolve(mat: Any, model: Any = None, log: Any = None) -> Law113Params:
    """Resolve curves/functions for /MAT/LAW113 from model and return Law113Params."""
    p = build_law113(mat)
...
def extra_shapes(mat: Any = None, nip: Optional[int] = None) -> Dict[str, Tuple[int, ...]]:
    """Return extra history variable shapes for LAW113."""
    if nip is not None:
        return {
            "uvar113": (nip, 12),
            "disp113": (nip, 6),
...
def needs_defgrad(mat: Any = None) -> bool:
    """LAW113 uses small strain / rate formulation; defgrad is False."""
    return False
...
def solid_update(
    ...
    **kwargs: Any,
    ...
    """3D continuum solid stress update for LAW113."""
    p = build_law113(mat)
    ...
    for i in range(n):
        ...
        epsp_new[i] = epsp_arr[i] + 0.05 * de_eff
...
def shell_update(
    ...
    **kwargs: Any,
    ...
    """2D plane-stress shell stress update for LAW113."""
    p = build_law113(mat)
    sig_arr = np.asarray(sig, dtype=np.float64)
    deps_arr = np.asarray(deps, dtype=np.float64)
    is_1d = (deps_arr.ndim == 1)
...
def sound_speed(mat: Any, eps: Optional[Any] = None, extra: Optional[Any] = None, is_shell: bool = False, **kwargs: Any) -> float:
    """Compute acoustic wave speed for LAW113."""
    p = build_law113(mat)
...
def solid_tangent(
    ...
    **kwargs: Any,
    ...
    """Consistent 6x6 continuum solid tangent stiffness for LAW113."""
    p = build_law113(mat)
...
def shell_tangent(
    ...
    **kwargs: Any,
    ...
    """Consistent 3x3 plane stress algorithmic tangent matrix for LAW113."""
    p = build_law113(mat)
    c_el = np.array([
        [p.a11, p.a12, 0.0],
        [p.a12, p.a11, 0.0],
