"""
Tsai-Wu quadratic failure criterion (/FAIL/TSAIWU).

Fortran origin: ``starter/source/materials/fail/tsaiwu/hm_read_fail_tsaiwu.F``,
``engine/source/materials/fail/tsaiwu/fail_tsaiwu_s.F`` (solids) and
``engine/source/materials/fail/tsaiwu/fail_tsaiwu_c.F`` (shells).
Failure model IRUPT = 44.

Theory:
-------
Tensor polynomial failure criterion for anisotropic materials:

Parameters:
  SIGMA_1T : Longitudinal tensile strength (X_t)
  SIGMA_2T : Transverse tensile strength (Y_t)
  SIGMA_12 : In-plane shear strength (S_12)
  SIGMA_1C : Longitudinal compressive strength (X_c)
  SIGMA_2C : Transverse compressive strength (Y_c)
  ALPHA    : Interaction coefficient (default 0.0)
...
Shell formula:
  A = F11*sxx**2 + F22*syy**2 + F66*sxy**2 + 2*F12*sxx*syy
  B = F1*sxx + F2*syy
  FINDEX = max(0, A + B)

Solid formula:
  A = F11*sxx**2 + F22*(syy**2 + szz**2) + F66*(sxy**2 + szx**2) + 2*F12*(sxx*syy + sxx*szz)
  B = F1*sxx + F2*(syy + szz)
  FINDEX = max(0, A + B)
...
def solid_step(fail, sig, d_epsp, deps, dt, dama, tstar=None):
    """Advance Tsai-Wu failure for a solid element slice; returns broken mask."""
    p = fail.params
    ...
    alpha = _get_param(p, ["alpha", "Alpha"], 0.0)
    ...
    f22 = 1.0 / (sigt2 * sigc2)
    f66 = 1.0 / (sigt12 ** 2)
    f12 = -0.5 * alpha * np.sqrt(max(0.0, f11 * f22))
    ...
    a = f11 * (sxx ** 2) + f22 * (syy ** 2 + szz ** 2) + f66 * (sxy ** 2 + szx ** 2) + 2.0 * f12 * (sxx * syy + sxx * szz)
    b = f1 * sxx + f2 * (syy + szz)
...
def shell_step(fail, sig, d_epsp, deps, dt, dama, tstar=None, eps_tot=None):
    """Advance Tsai-Wu failure for a shell layer; returns broken mask."""
    p = fail.params
    sigt1 = max(_get_param(p, ["sigma_1t", "Sigma_1t", "xt", "Xt", "s1t"], _INF), _TINY)
    sigt2 = max(_get_param(p, ["sigma_2t", "Sigma_2t", "yt", "Yt", "s2t"], _INF), _TINY)
    sigc1 = max(_get_param(p, ["sigma_1c", "Sigma_1c", "xc", "Xc", "s1c"], _INF), _TINY)
    ...
    a = f11 * (sxx ** 2) + f22 * (syy ** 2) + f66 * (sxy ** 2) + 2.0 * f12 * (sxx * syy)
    b = f1 * sxx + f2 * syy
    findex = np.maximum(0.0, a + b)
