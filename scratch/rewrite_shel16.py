...
def rewrite_shell_thick16():
    with open("pyradioss/elements/shell_thick16.py", "r") as f:
        content = f.read()
        
    # We will append the _pre and _post functions and forces()
    # First check if we already added it
    if "def _s16_pre" in content:
        print("Already added")
        return
...
"""
    ...
    # We need to add this and rewrite init_group
    # Let's just output this code and we can manually copy paste
    with open("scratch/s16_forces.py", "w") as out:
        out.write(code_to_add)

rewrite_shell_thick16()
