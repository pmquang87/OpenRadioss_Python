"""M41 — the RD-E-1000 Sf_0.1 energy-guard startup regression (task_29ec1751).

The finding (VALIDATION.md M40 §3.5)
-----------------------------------
...
port's −15 % energy-error guard, against **~1e-8 absolute energies** (IE
...
IE+KE reads as a huge percentage (ERR −84 % at cycle 1, −17 % at cycle 100),
...
(the printed percentage is additionally clamped to ±99.9 %, X99), and the stop
thresholds DEMXK/DEMXS default to EP20/EP30 when the deck sets no /STOP limit
(freform.F 782/803) — i.e. upstream does not abort on energy error by default
...
floor (``engine._ENERGY_START_FLOOR``).  The 15 %/30 % limits are UNCHANGED —
...
* a near-zero-energy startup deck runs cleanly past the cycle-100 check (the
  c40/c42/c44 behaviour), and WITHOUT the floor the same deck aborts there —
  so the conditioning is load-bearing, not a silent no-op;
* the guard RE-ARMS once the reference energy is real — a genuine imbalance
...
* the M39-era ``h = -1`` hourglass 'teeth' negative control still detonates
  under the default 15 % limit — a real instability is still caught.
...
KE term (translational ½·m·v² only — exactly like the Fortran ENCIN, whose
rotational ENROT is a separate channel) does not count, so the run carries a
large NEGATIVE %-error (ERR → −100 %) at near-zero ABSOLUTE energy: the
...
# _ENERGY_START_FLOOR dust threshold — REF ≈ 4.9e-9 at cycle 100, crossing
...
def _drill_shell_starter(fscale=_DRILL_FSCALE):
    ...
    external work → uncounted rotational KE → large %-error at dust energy."""
    return (
        "/BEGIN\nm41 drill shell\n"
        "/NODE\n1 0 0 0\n2 1 0 0\n3 1 1 0\n4 0 1 0\n"
        "/SHELL/1\n1 1 2 3 4\n"
...
def _teeth_brick_starter(hg):
    ...
    NEGATIVE hourglass coefficient (h = −1) turns the Flanagan–Belytschko
    viscous damper into an amplifier — a genuinely unstable run whose energy
...
def _ref_of(row):
    ...
    listing row — the max of the energy magnitudes (see engine._energies).
...
def test_near_zero_energy_startup_runs_past_cycle_100(make_deck):
    ...
    (REF < the floor, |ERR| ≈ 100 % on that dust) now runs cleanly past the
    ...
    # ran clean to the (truncated) end — no guard abort during startup
...
def test_without_the_floor_the_same_deck_aborts_at_cycle_100(make_deck,
    ...
    100) on ~1e-9 absolute energy — proving the deck genuinely exercises the
    ...
    # the abort fired on dust — the near-zero denominator the fix conditions out
    assert _ref_of(rows[-1]) < PROD_FLOOR, rows[-1]


# ---------------------------------------------------------------------------
# The guard is CONDITIONED, not disabled — it re-arms above the floor
# ---------------------------------------------------------------------------
...
def test_guard_rearms_once_reference_energy_clears_the_floor(make_deck):
    ...
    the floor, and the energy-error guard fires again — a real (non-dust)
    ...
    model, rows = _run(make_deck, "MREARM", _drill_shell_starter(),
                       "/RUN/MREARM/1\n1.0\n/DT\n0.9 0\n/PRINT/-100\n")
    assert "ENERGY ERROR" in model.engine_state.stop_reason
    # it did NOT abort in the dust band — it survived startup and only tripped
...
def test_teeth_negative_control_still_detonates(make_deck):
    """The M39-era negative control at the guard level: the h = −1 hourglass
    'teeth' brick (a genuine instability) must STILL trip the divergence guard
    under the default 15 % limit — the floor conditioning must not blind the
    guard to real energy injection.  (REF crosses the floor at cycle 1; the
    numerical-injection guard fires by cycle 3 at ERRN ≈ −50 %.)"""
    ...
    # an ENERGY guard caught it (not merely a dt collapse) — the guard is live
    # above the floor
...
def test_teeth_positive_control_stays_balanced(make_deck):
    ...
    coefficient (h = +0.1) runs to the end with no guard abort — the floor
