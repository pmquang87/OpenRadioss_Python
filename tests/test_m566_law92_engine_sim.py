"""
Dynamic Engine Simulation & Energy Balance Tests for Milestone M566 (/MAT/LAW92 Arruda-Boyce).
"""
...
class TestLaw92EngineSimulation:
    """Multi-cycle dynamic explicit engine simulations for LAW92."""

    def test_solid_hexa8_arruda_boyce_tensile_energy_balance(self, tmp_path: Path):
        """Single Hexa8 brick with /MAT/LAW92 under tensile stretching."""
        run_name = "HEXA8_LAW92"
        ...
        d.mat_law92(
            1, "Arruda_Rubber",
            rho0=1.0e-9, mu=5.0, d=1.0e-3, lam=5.0, nu=0.495,
        )
    ...
    def test_shell_bt4_arruda_boyce_tensile_loading(self, tmp_path: Path):
        """Single 4-node shell with /MAT/LAW92 under in-plane stretching."""
        run_name = "SHELL_LAW92"
        s_path = str(tmp_path / f"{run_name}_0000.rad")
        e_path = str(tmp_path / f"{run_name}_0001.rad")
        ...
        d.mat_law92(
            1, "Arruda_Shell",
            rho0=1.0e-9, mu=4.0, d=1.0e-3, lam=5.0, nu=0.49,
        )
        ...
        d.funct(11, "vx_pull", [(0.0, 100.0), (5.0e-4, 100.0)])
        d.impvel(1, "pull_x", 11, "X", 2)

        d.write(s_path)
        _write_engine_deck(e_path, run_name, tstop=2.0e-3, dt_scale=0.5, stop_cycles=50)
